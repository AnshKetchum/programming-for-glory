...
int main()
{
    ...
    for(int i = 0; i < n; i++)
    {
        for(int j = 0; j < m; j++)
        {
            int wallsInBits = 0; fin >> wallsInBits;
            int powerOf2 = 1;
            for(int a = 0; a < 4; a++)
            {
    ...
    fout << highestRegion << endl;
    fout << highestWithTwoRegions << endl;
    fout << lowestCoord.first + 1 << " " << lowestCoord.second + 1 << " " << wallPlacement << endl;
